...
- **整会话导出** — 在会话头部点击「⬇ 导出」，可将整个会话导出为 **JSON**（无损，保留所有元数据）或 **Markdown**（简洁版 / 详细版）。
- **单条消息导出** — 鼠标悬停在任意消息气泡上，右上角出现 `↓ md` 小按钮；点击即下载该条消息为独立 Markdown 文件。完全在浏览器端完成，无需服务器往返。
- JSON 导出包含会话统计：总 token 数、各工具调用次数、各角色消息数。
- Markdown 详细版将 `thinking` 块包裹在 `<details>` 折叠组件中并展开完整工具输入/输出；简洁版将其压缩。
